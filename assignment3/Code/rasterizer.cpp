...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    for (const auto& t:TriangleList)
    {
        ...
        // viewspace_pos is the camera coordinate of vertices.
        // newtri is the projected triangle
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    // for(int y = 0; y < height; ++y)
    {
        if(insideTriangle(x, y, &v[0]))
        {
            ...
            z_interpolated /= weight_sum;
            // std::cout<<z_interpolated<<std::endl;
            // z_interpolated *= w_reciprocal; 
            int id =  get_index(x, y);
            if(z_interpolated < depth_buf[id])
            {
                // std::cout<<z_interpolated<< " "<<depth_buf[id]<<std::endl;
                // interpolate to get color, normal, tex_coor, and shading
                ...
                // interpolated_shadingcoords.setZero();
